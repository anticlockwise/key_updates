...
class TkcScraper(PyQueryBasedScraper):
    ...
    def _scrape(self, doc):
        ...
        for product_update_item in product_update_items:
            ...
            item_status_elem = item_pq(
                ".list-product__description .list-product__status"
            )
            item_status = item_status_elem.text().replace(STATUS_LABEL, "").strip()

            expected_ship_date_elem = item_pq(
                ".list-product__description .list-product__ship-date"
            )
            expected_ship_date = normalize_date(
                expected_ship_date_elem.text()
                .replace(EXPECTED_SHIP_DATE_LABEL, "")
                .strip()
            )
